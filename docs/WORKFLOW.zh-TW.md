...
> **效能說明**：錄製代理採用 WriteStream 架構，對目標應用的額外延遲 < 1ms，可在 100+ QPS 的正常環境持續收集，不影響應用程式回應速度。執行 `archivolt record stop` 時，代理會等待所有資料完全落地後才標記 stopped，確保資料完整。
...
> **注意**：停止後抵達的遲到資料（如 HTTP proxy 尚在傳輸的 chunk）會被靜默忽略，不影響已落地的資料。
...
```bash
# 最簡單：只跑離線 Pattern 分析（不需 live DB）
archivolt analyze <session-id> --format optimize-md

# 加入 DDL Schema 對比（偵測索引缺口）
archivolt analyze <session-id> --format optimize-md --ddl ./schema.sql

# 加入 EXPLAIN 活體驗證（需連線到 DB）
archivolt analyze <session-id> --format optimize-md \
  --ddl ./schema.sql \
  --explain-db mysql://user:pass@localhost:3306/mydb

# 輸出到 stdout
archivolt analyze <session-id> --format optimize-md --stdout
```

#### 三層分析架構

分析分三層，每層可獨立啟用，缺少任一層不影響其他層運作：

| 層 | 啟用條件 | 分析內容 |
|----|----------|---------|
| **Layer 1: Pattern**（永遠執行）| 無 | 讀寫比分析、N+1 偵測、查詢碎片化偵測 |
| **Layer 2a: DDL Diff** | `--ddl <schema.sql>` | 對比 DDL 現有索引 vs. 查詢 WHERE 欄位，找出索引缺口 |
| **Layer 2b: EXPLAIN** | `--explain-db <url>` | 對每個 unique query 執行 EXPLAIN，偵測全表掃描 |

#### 報告產出內容

```
## 📊 讀寫比分析          — 各資料表讀寫次數與比例，並建議快取策略
## 🔴 N+1 問題            — 每個 API endpoint 的重複查詢，附批次查詢改寫 SQL
## 🟡 查詢碎片化          — 同一請求內同 pattern 重複 3+ 次的查詢
## 🟠 索引缺失            — DDL 對比後發現的索引缺口（附 CREATE INDEX）
## 🔴 全表掃描            — EXPLAIN 確認的 type=ALL 查詢（附 CREATE INDEX）
```

每條建議都附上可直接 copy-paste 的 SQL 指令。

#### 進階選項

| 選項 | 預設值 | 說明 |
|------|--------|------|
| `--min-rows <n>` | 1000 | EXPLAIN 全表掃描觸發的最小估計行數 |
| `--explain-concurrency <n>` | 5 | 同時執行 EXPLAIN 的並發數（VPN 環境建議降低） |

**連線逾時處理**：若 `--explain-db` 連線失敗（逾時 5 秒），Layer 2b 自動跳過，報告中標注警告，不影響 Layer 1 / 2a 的結果。
...
| `archivolt analyze <id>` | 語義分析（Operation Manifest） |
| `archivolt analyze <id> --format optimize-md` | 效能診斷報告（Layer 1 離線分析） |
| `archivolt analyze <id> --format optimize-md --ddl <schema.sql>` | 效能診斷 + DDL 索引缺口分析 |
| `archivolt analyze <id> --format optimize-md --explain-db <url>` | 效能診斷 + EXPLAIN 全表掃描偵測 |
