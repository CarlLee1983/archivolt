...
1. **啟動錄製代理**：
   ...
   ```
2. **搭配瀏覽器擴充功能**：安裝 Archivolt Chrome Extension，它會在您操作畫面時自動送出行為標記（Marker），這對於後續將 SQL 與具體動作（如「點擊結帳」）關聯至關重要。
3. **切換連線**：將您的應用程式（例如 Laravel 的 `.env`）的資料庫 Host 改為 `127.0.0.1`，Port 改為 `13306`。
4. **執行業務流程**：在瀏覽器操作您想分析的功能。

### 3. 語義分析 (Operation Manifest)
錄製完成後，您可以對該次 Session 進行深度分析，產出一份結構化的中間文件：
1. **執行分析命令**：
   ```bash
   bun run dev analyze <session-id>
   ```
2. **產出物 (Operation Manifest)**：
   這會在 `data/analysis/<session-id>/manifest.md` 產生一份文件，包含：
   - **操作清單**：按時間與行為將 SQL 分組，並推斷機械式語義（如 `INSERT orders; UPDATE inventory`）。
   - **Table Matrix**：視覺化呈現各資料表在流程中的讀寫參與度。
   - **關係推斷**：自動識別 `JOIN` 或 `WHERE IN` 產生的虛擬外鍵建議（標註信心等級）。
3. **用途**：
   - **AI 驅動開發**：這是 Claude Code 或 Cursor 的最佳「導航地圖」，讓 AI 能理解複雜的業務邏輯並準確生成代碼。
   - **重構依據**：作為舊系統行為模式的正式存檔與分析報告。
