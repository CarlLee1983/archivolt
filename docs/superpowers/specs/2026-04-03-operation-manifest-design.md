...
**優化說明：**
- **支援空 Chunk**：即使某個 marker（如 `navigate`）沒有觸發任何 DB query，也會保留該 chunk（`queries` 陣列為空），以便完整呈現使用者的操作脈絡。
- **語義精準化**：語義推斷會按操作精確匹配對應的 table（如 `INSERT orders; UPDATE inventory`），而非簡單地羅列所有操作與 table。
...
for each chunk:
  ...
  4. 根據 pattern + queries + marker 產生語義標籤和描述
    |
...
- **無 marker** -> 標註為 `(silence-based split)`
- **語義推斷**：
  - 按 SQL 操作類型分組對應的 Table。
  - 無 Query 時顯示 `(no database operations)`。
  - 範例：`SELECT products, categories; INSERT orders`。
