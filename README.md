...
- **Performance Optimization Report** (`--format optimize-md`): Four-layer analysis pipeline. Layer 1 runs offline from recorded sessions: per-table read/write ratios with Redis/Read Replica recommendations, N+1 query detection aggregated to API path level, and query fragmentation detection. Layer 2a adds DDL schema diff to detect un-indexed WHERE columns (`--ddl`). Layer 2b connects to a live database to confirm full table scans via EXPLAIN (`--explain-db`). Layer 3 (`--llm`) sends the top-N highest-impact findings to Claude Haiku for actionable, context-aware recommendations — each one grounded in your actual schema and read/write profile. Every finding includes a runnable SQL snippet — `CREATE INDEX`, batch query rewrite, or cache comment — ready to copy-paste.
